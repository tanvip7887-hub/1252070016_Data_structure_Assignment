...
A Binary Tree is a hierarchical structure where each node can have at most two children: left and right.
Non-recursive traversals use a stack instead of recursion to visit tree nodes step by step.
Inorder Traversal (Left → Root → Right) displays the binary tree nodes in sorted order for BSTs.
Preorder Traversal (Root → Left → Right) is used to create a copy of the tree or prefix expression.
Leaf nodes are nodes that have no left and no right child; they represent endpoints of the tree.
Mirror Image of a tree is created by swapping the left and right child of every node using stack-based traversal.
...
C:\Users\USER\OneDrive\Desktop\VS_DS_CPP>g++ ASS34.cpp
