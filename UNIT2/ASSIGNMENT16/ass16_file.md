...
A Generalized Linked List (GLL) is a special linked list that can store both simple elements and entire sublists inside the same structure. It
is useful for representing nested sets because each node can either be an atom (single value) or a list (another GLL). In this program, the set 
S with nested elements is stored using GLL nodes that connect atoms and inner lists together. Recursive functions are used to display the set 
in proper set notation. This structure makes it easy to represent complex, multi-level sets in a flexible way.
...
GLL Set S = {p, q, {r, s, t, {}, {u, v}, w, x, {y, z}, a1, b1}}
